...
## Demo — Actions: build → release → aprobación

Esta parte muestra cómo Actions **empaqueta** un release y lo **promueve** entre ambientes
con una **aprobación manual** antes de Producción.

> Requisito previo (paso manual en la UI): crear los Environments `qa` y `production`, y en
> `production` agregar **Required reviewers**. Ver la sección de pasos manuales del repositorio.

1. **(1 min) Disparar el release** — pestaña **Actions** → workflow **release** → botón
   **Run workflow** (`workflow_dispatch`). Alternativa: crear la tag `v1.0.0` (`git tag v1.0.0`
   + `git push origin v1.0.0`), que dispara el mismo workflow.
2. **(1 min) Empaquetado** — ver el job **Empaquetar release**: hace `dotnet publish` y genera
   el artefacto `inventory-app-<versión>.zip`. Descargarlo desde la sección **Artifacts** de la
   ejecución para mostrar el "paquete distribuible".
3. **(1 min) QA automático** — el job **Desplegar en QA** corre solo (sin gate) e imprime
   *"Desplegando versión X en QA"*.
4. **(2 min) Gate de Producción** — el job **Desplegar en Producción** queda **EN ESPERA**
   (*Waiting*), pausado por el **Required reviewer**. Mostrar que **nada llega a prod** sin
   aprobación humana. Aquí se conecta con GHAS: la seguridad debe estar en verde antes de aprobar.
5. **(1 min) Aprobar** — hacer clic en **Review deployments** → marcar `production` → **Approve
   and deploy**. El job se reanuda e imprime *"Desplegando versión X en Producción"*.
6. **Cierre** — "el pipeline construye una vez y promueve el mismo artefacto; la promoción a
   producción exige una decisión humana".
